...
def get_fields(dt_dump):
    ...
    for field in s:
        current_field = {'name': "", 'type': "", 'pointer': 0, 'offset': -
                         1, 'size': 0, 'array_sizes': [], 'union': False, 'bit_pos': -1}
        ...
        if len(field_list) > 0:
            if field_list[-1]['bit_pos'] == -1 and current_field['bit_pos'] == -1:
                ...
                field_list[-1]['size'] = current_field['offset'] - \
                    field_list[-1]['offset']
...
def print_bitfield(bit_field, bit_field_type):
    printf("\t%s %s : %d; // 0x%X\n", bit_field_type,
           bit_field['name'], bit_field['size'], bit_field['offset'])
